...
const handlers = {
  ...
  onRender: ({ source, schema, onSave }) => (dispatch, getState) => {
  ...
  onMoveFieldEnd: ({ oldIndex, newIndex }) => ({ type: 'MOVE_FIELD', oldIndex, newIndex }),

  onAddFieldClick: () => ({ type: 'ADD_FIELD' }),

  // EditorField

  onRemoveFieldClick: (columnId) => ({ type: 'REMOVE_FIELD', columnId }),

  onFieldPropertyChange: (columnId, key, value) => ({ type: 'UPDATE_FIELD', columnId, key, value }),
}
...
const mutations = {
  ...
  SET_RENDER: (state, { onSave }) => {
    state.loading = true
    state.onSave = onSave
  },

  SET_LOAD_SUCCESS: (state, { columns, metadata }) => {
    state.loading = false
  ...
  MOVE_FIELD: (state, { oldIndex, newIndex }) => {
  ...
  REMOVE_FIELD: (state, { columnId }) => {
    state.columns = state.columns.filter((column) => column.id !== columnId)
  },

  UPDATE_FIELD: (state, { columnId, key, value }) => {
